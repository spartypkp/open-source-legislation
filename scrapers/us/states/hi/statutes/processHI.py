...
def fix_bad_node_id():
    ...
    select_sql = "SELECT node_id, node_type, node_text, node_citation, node_name, node_addendum, node_tags FROM hi_node WHERE node_type ILIKE '%content%' and node_id LIKE '%§%';"
    selected_rows = util.select_and_fetch_rows(conn, select_sql)

    # Selected rows is a list of tuples
    for row in selected_rows:
        ...
        print(f"== Old id: {node_id} ==")
        print(node_text)

        temp = node_text[0].replace("‑","-").split("-")[1]
        print(f"TEMP: ", temp)
        new_node_number = temp.split(" ")[0].replace("]", "")
        
        
        #find the last occurence of = in node_id
        last_equal_index = node_id.rfind("=")
       
        new_node_name_temp = temp.split(".")[0]
        first_space_index = new_node_name_temp.find(" ")
        new_node_name = new_node_name_temp[first_space_index+1:] + "."
        

        #replace everything after that with the new node number
        new_node_id = node_id[:last_equal_index+1] + new_node_number

        
        # Find the chapter_num from node_id. Should be after CHAPTER=
        chapter_num = node_id.split("CHAPTER=")[1].split("/")[0]
        new_node_citation = f"HRS § {chapter_num}-{new_node_number}"

        new_node_name = f"§{chapter_num}-{new_node_number}{new_node_name}"
        if node_text and "[" in node_text[-1] and "]" in node_text[-1]:
            new_node_addendum = node_text[-1].split("[")[1].split("]")[0]
        
        print(f"New node_number: {new_node_number}")
        print(f"New node_name: {new_node_name}")
        print(f"New node_id: {new_node_id}")
        print(f"New node_citation: {new_node_citation}")
        print(f"New node_addendum: {new_node_addendum}")

        # Escape single quotes for SQL
        new_node_name = new_node_name.replace("'", "''")
        
        new_node_addendum = new_node_addendum.replace("'", "''")

        # Update the row with the new node_id
        
        cursor = conn.cursor()
        try:
            update_sql = f"UPDATE hi_node SET node_id = '{new_node_id}', node_name = '{new_node_name}', node_citation = '{new_node_citation}', node_type = 'content', node_addendum = '{new_node_addendum}' WHERE node_id = '{node_id}';"
            cursor.execute(update_sql)
            conn.commit()
        except psycopg2.errors.UniqueViolation:
            conn.rollback()
            # If the updated row already exists, just delete the old row
            delete_sql = f"DELETE FROM hi_node WHERE node_id = '{node_id}';"
